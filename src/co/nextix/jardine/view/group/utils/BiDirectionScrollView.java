...
public class BiDirectionScrollView extends FrameLayout {
	public static class AndroidUtils {
		...
		public static float dipToPixels(final Context _context,
				final float _dipValue) {
			final DisplayMetrics metrics = _context.getResources()
					.getDisplayMetrics();
	...
	private class BiDirectionScrollViewFlinger implements Runnable {
		...
		void start(final int _velocityX, final int _velocityY) {
			...
			scroller.fling(0, 0, velocityX, velocityY, -Integer.MAX_VALUE,
	...
	public BiDirectionScrollView(final Context context) {
		...
		scrollThresholdDistance = AndroidUtils.dipToPixels(getContext(),
				SCROLL_THRESHOLD_DISTANCE_DIP);
	}
	...
	private GestureDetector createScrollGestureDetector() {
		final GestureDetector res = new GestureDetector(
				new GestureDetector.SimpleOnGestureListener() {
					...
					public boolean onScroll(final MotionEvent e1,
						...
						if (currentScrollGestureBroken
								|| !currentlyIntoScrollGesture) {
							return false;
						}

						BiDirectionScrollView.this.scrollBy(
								currentScrollGestureYOverride ? 0
										: (int) distanceX,
								currentScrollGestureXOverride ? 0
										: (int) distanceY);
						return true;
					}
	...
	private void fixScrollIfOutOfBounds() {
		...
		if (childHeight > getHeight()
				&& getScrollY() + getHeight() > childHeight) {
	...
	public boolean dispatchTouchEvent(final MotionEvent ev) {
		if (ev.getAction() == MotionEvent.ACTION_DOWN) {
			...
			if (!currentlyIntoScrollGesture
				...
				currentScrollGestureXOverride = scrolledYDistance
						* SCROLL_DIRECTION_OVERRIDE_FACTOR < scrolledXDistance;
				currentScrollGestureYOverride = scrolledXDistance
						* SCROLL_DIRECTION_OVERRIDE_FACTOR < scrolledYDistance;
